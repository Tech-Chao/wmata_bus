...
class _BusStopPageState extends State<BusStopPage> {
  ...
  Future<void> _loadAutoRefresh() async {
    ...
    setState(() {
      autoRefresh = tempAutoRefresh == null ? false : bool.parse(tempAutoRefresh);
    });
  ...
  Future<void> fetchPredictions() async {
    ...
    try {
      final predictions = await APIService.getBusPredictions(stpid: selectedStop!.stopID!);
  ...
  Widget build(BuildContext context) {
    ...
    final descriptionTitle = (widget.route?.name ?? "").split(' - ').sublist(1).join(' - ');

    final directionModel = direction ? routeDetail?.direction0 : routeDetail?.direction1;
    ...
    final segmentedWidgets = Map.fromEntries(
      directionNames.map((e) => MapEntry(
        e,
        Padding(
    ...
    return Scaffold(
      ...
      body: _buildBody(context, directionModel, segmentedWidgets, destinationName, favorStops),
      floatingActionButton: _buildFloatingActionButton(context),
    );
  }

  PreferredSizeWidget _buildAppBar(BuildContext context, String title, String subtitle) {
    return AppBar(
      title: Column(
  ...
  Widget _buildBody(BuildContext context, Direction? directionModel, 
      Map<String, Widget> segmentedWidgets, String? destinationName, List<BusStop> favorStops) {
    if (isLoading) {
      return const Center(child: CupertinoActivityIndicator());
    }

    if (directionModel?.stops == null || directionModel?.stops!.isEmpty == true) {
      return Center(
        child: Text(
  ...
  Widget _buildDirectionSelector(BuildContext context, Map<String, Widget> segmentedWidgets, Direction? directionModel) {
    return Padding(
      ...
      child: segmentedWidgets.length > 1
          ...
          : Text(
              directionModel?.directionText ?? "",
              style: Theme.of(context).textTheme.titleMedium
            ),
    );
  }

  Widget _buildDestinationHeader(BuildContext context, String? destinationName) {
    return Text(
      'TO $destinationName',
  ...
  Widget _buildStopsList(BuildContext context, Direction? directionModel, List<BusStop> favorStops) {
    return Expanded(
  ...
  void _handleFavorite(BuildContext context, BusStop stop, List<BusStop> favorStops) {
  ...
  Future<void> _initializeAd() async {
    ...
    final adUnitId = kDebugMode
        ? (Platform.isAndroid ? ConstTool.kAndroidDebugBannerId : ConstTool.kiOSDebugBannerId)
        : (Platform.isAndroid ? ConstTool.kAndroidReleaseBannerId : ConstTool.kiOSReleaseBannerId);
